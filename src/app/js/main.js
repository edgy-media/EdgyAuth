...
var updateTotp = function(secret, el) {
  for (var i = 0; i < storage.accounts.length; i++) {
      ...
      if (config.debug == true) {
          console.log(i)
          console.log(`${totp.getOtp(account.secret)}   ${account.label}`)
      }
      document.getElementById("account-" + account.id).innerHTML = `<button type="button" class="btn btn-outline-info token" onclick="copyCode('${totp.getOtp(account.secret)}')"><h1 class="">${totp.getOtp(account.secret)}</h1></button>`;
  }
  if (config.debug == true) {
      console.log(totp.getOtp(secret))
  }
